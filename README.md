...
- 桌面通知开关：工具栏复选框 / 托盘菜单一键开关告警气泡，开关状态持久化
...
- 图例交互：单击切换曲线显隐、双击 solo 单条曲线，显隐状态持久化；表格行带曲线同色色块
- 表格首列"显示"复选框：直接勾选目标是否绘入图表，与图例单击/双击 solo 双向同步，状态持久化
- 目标排序：右键菜单"上移/下移"或 Alt+↑/↓ 调整顺序，重启保持
